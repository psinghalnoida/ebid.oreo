...
<label style="font-size:12px; color:var(--ink-3);">Fee Payer Election (BR-31/32) — locked once bidding is live</label>
<select name="fee_payer" style="display:block; width:100%; padding:12px; margin:6px 0 14px; border:1px solid var(--line); border-radius:10px;">
  <option value="buyer_pays">Buyer-Pays (default) — buyer's EMD covers the Success Fee</option>
  <option value="seller_pays" <?= empty($sellerPaysAllowed) ? 'disabled' : '' ?>>Seller-Pays — you absorb the Success Fee, buyer sees no premium<?= empty($sellerPaysAllowed) ? ' (requires a paid TSX tier)' : '' ?></option>
</select>
