...
<select name="fee_payer" style="display:block; width:100%; padding:12px; margin:6px 0 14px; border:1px solid var(--line); border-radius:10px;">
  ...
  <option value="seller_pays" <?= empty($sellerPaysAllowed) ? 'disabled' : '' ?>><?= tsx_term('Seller') ?>-Pays — you absorb the Success Fee, <?= strtolower(tsx_term('Buyer')) ?> sees no premium<?= empty($sellerPaysAllowed) ? ' (requires a paid TSX tier)' : '' ?></option>
</select>
