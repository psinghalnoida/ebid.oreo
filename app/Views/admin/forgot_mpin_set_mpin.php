...
<main style="max-width:420px; padding:60px 24px;">
  <h1 style="font-size:24px;">Set a New mPIN</h1>
  <p style="color:var(--ink-2); font-size:14px;">Both codes verified. Choose a new 4-digit mPIN.</p>
  <?php if (!empty($error)): ?>
    <p style="color:#B5482F; font-size:13px;"><?= esc($error) ?></p>
  <?php endif; ?>
  <form method="post" action="/admin/forgot-mpin/set-mpin"><?= csrf_field() ?>
