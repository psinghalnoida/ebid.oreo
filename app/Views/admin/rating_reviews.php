...
<main style="max-width:820px; padding:40px 24px;">
  ...
  <p style="font-size:12px; color:var(--ink-3);">Downgrades between 5.0 and 2.1★ need Tenant Admin approval; at 2.0★ or below, both Tenant Admin and Super Admin.</p>
  ...
  <?php foreach ($pending as $e): ?>
    <div style="border:1px solid var(--line); border-radius:12px; padding:16px; margin-top:10px;">
      <p style="font-size:13px; font-weight:700; margin:0 0 4px;">
        <?= esc($e['mobile_number']) ?> — <?= esc($e['rating_role'] === 'star_rating' ? 'Buyer' : 'Seller') ?> rating
      ...
      <p style="font-size:11px; color:var(--ink-3); margin:0 0 10px;">
        <?= $e['tenant_admin_approved_at'] ? '✓ Tenant Admin approved' : 'Awaiting Tenant Admin' ?>
        <?= (float) $e['new_value'] <= 2.0 ? (' · ' . ($e['super_admin_approved_at'] ? '✓ Super Admin approved' : 'Awaiting Super Admin')) : '' ?>
      </p>
