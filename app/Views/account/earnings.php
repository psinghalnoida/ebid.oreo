...
<main style="max-width:600px; padding:40px 24px;">
  ...
  <div style="display:flex; gap:12px;">
    ...
    <div style="flex:1; border:1px solid var(--line); border-radius:12px; padding:14px;">
      <p style="font-size:20px; font-weight:800; margin:0;">₹<?= number_format((float) $ytd['total_sales'] - (float) $ytd['total_fees'], 2) ?></p>
      <p style="font-size:11px; color:var(--ink-3);">Net Received</p>
    </div>
    <div style="flex:1; border:1px solid var(--line); border-radius:12px; padding:14px;">
  ...
  <p style="font-size:11px; color:var(--ink-3); margin-top:8px;">
    "Net Received" reflects the platform fee split (BR-33) only — actual payout to your bank account requires the
    real payment gateway, which is not yet connected (deferred to post-deployment, per the project's own decision).
  </p>
</main>
<?= $this->endSection() ?>
