...
<main style="max-width:420px; padding:60px 24px;">
  <h1 style="font-size:24px;">Confirm It's You</h1>
  <p style="color:var(--ink-2); font-size:14px;">BR-50: OTP re-verification before this payout bank account change is accepted.</p>
  <?php if (!empty($devOtp)): ?>
  ...
  <?php endif; ?>
  <form method="post" action="/payout-account/change/verify-otp">
    ...
    <button type="submit" class="btn btn-emerald" style="width:100%;">Verify & Save</button>
  </form>
</main>
<?= $this->endSection() ?>
