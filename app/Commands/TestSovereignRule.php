...
class TestSovereignRule extends BaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        CLI::write("\n=== Step 1: listAll() seeds the 7 wired rules at their exact original hardcoded values ===", 'yellow');
        $all = $rules->listAll();
        $byKey = [];
        foreach ($all as $r) {
            if ($r['rule_key']) $byKey[$r['rule_key']] = $r;
        }
        $this->assert(count($byKey) === 7, 'All 7 wired rules are seeded');
        ...
        $this->assert((float) $byKey['TechStack-3.10.server_time_drift_tolerance_seconds']['numeric_value'] === 5.0, 'Server time drift tolerance seeded at the original 5.0s');
        ...
        // These 7 rules are genuinely live, platform-wide, and persist in
        ...
        CLI::write("\n=== Teardown: restoring all 7 wired rules to their original values ===", 'yellow');
        ...
        $this->assert(
            ...
            && $restored['TechStack-3.10.server_time_drift_tolerance_seconds'] === 5.0,
            'All 7 rules restored to their original values — this test leaves no live config changed behind'
        );
